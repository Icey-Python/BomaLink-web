...
const OurStory = ({tagId}:Props) => {
  ...
  return (
    <section className="w-screen h-[100vh] bg-[#D6F9C6] text-white font-poppins flex-col relative" id={tagId}>
    <span className="absolute text-2xl md:text-4xl lg:text-6xl font-bold text-white/5 z-0  ">{name}</span>
    <Marquee className="invisible lg:visible" pauseOnHover={true} style={{position:"absolute",top:"50%",overflowY:"hidden",transform:"translateY(-50%)",width:"100%",height:"fit-content",zIndex:"10"}} speed={15} autoFill={true}>
    ...
    <p className="text-center mx-auto my-auto lg:my-0 w-fit text-4xl md:text-5xl lg:text-7xl">Coexistence between  <br/> farmers and pastoralists</p>
    <span className="absolute top-1/3 left-1/4 text-4xl md:text-7xl lg:text-9xl font-bold text-white/5 z-0">{name}</span>
    <span className="absolute top-3/4 right-1 text-2xl md:text-4xl lg:text-6xl font-bold text-white/5 z-0">{name}</span>
   </div> 
